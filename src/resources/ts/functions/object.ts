 ...
 */
export function indexify<T>(object: T): T {
    if (Array.isArray(object)) {
        const prefix = Math.round(Date.now() / 1000)
        object.forEach((v, k) => {
            if (typeof v === 'object' && v !== null) {
                (v as Record<string, unknown>)._id = prefix + k.toString()
                indexify(v) // On applique récursivement
            }
        })
...
/**
 * Converts the value to the same type as the second parameter
 */
